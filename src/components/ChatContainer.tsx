...
export default function ChatContainer({ currentChat, socket }: chatCon) {
  ...
  const handleSendMsg = async (msg: any) => {
    // 获取当前用户对象
    const data = await JSON.parse(
      String(localStorage.getItem(String(process.env.REACT_APP_LOCALHOST_KEY)))
    );
    //socket发送信息
